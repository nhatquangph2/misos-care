  ...
  async function loadDashboardData() {
    try {
      ...
      setStats({
        ...
        big5Profile: personality ? {
          extraversion: (personality as any).big5_extraversion,
          agreeableness: (personality as any).big5_agreeableness,
          conscientiousness: (personality as any).big5_conscientiousness,
          neuroticism: (personality as any).big5_neuroticism,
          openness: (personality as any).big5_openness,
          lastUpdated: new Date((personality as any).last_updated).toLocaleDateString('vi-VN'),
        } : undefined,
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                ...
                Hồ Sơ Tính Cách BIG5
              </CardTitle>
              <Link href="/tests/big5/results">
            ...
            <CardDescription>
              Cập nhật lần cuối: {stats.big5Profile.lastUpdated}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="text-center">
                <div className="text-sm text-gray-600 mb-1">Hướng Ngoại</div>
                <div className="text-2xl font-bold text-purple-600">
                  {stats.big5Profile.extraversion?.toFixed(1) || '—'}
                </div>
                <div className="text-xs text-gray-500">/5.0</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-600 mb-1">Dễ Chịu</div>
                <div className="text-2xl font-bold text-blue-600">
                  {stats.big5Profile.agreeableness?.toFixed(1) || '—'}
                </div>
                <div className="text-xs text-gray-500">/5.0</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-600 mb-1">Tận Tâm</div>
                <div className="text-2xl font-bold text-green-600">
                  {stats.big5Profile.conscientiousness?.toFixed(1) || '—'}
                </div>
                <div className="text-xs text-gray-500">/5.0</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-600 mb-1">Bất Ổn</div>
                <div className="text-2xl font-bold text-orange-600">
                  {stats.big5Profile.neuroticism?.toFixed(1) || '—'}
                </div>
                <div className="text-xs text-gray-500">/5.0</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-600 mb-1">Cởi Mở</div>
                <div className="text-2xl font-bold text-indigo-600">
                  {stats.big5Profile.openness?.toFixed(1) || '—'}
                </div>
                <div className="text-xs text-gray-500">/5.0</div>
              </div>
            </div>
