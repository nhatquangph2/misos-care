...
export default async function GamificationPage() {
    ...
    const handleBuy = async (formData: FormData) => {
        ...
        // Toast handled by client component ideally, but server action revalidates
    };

    return (
        <div className="container p-6 space-y-8 max-w-6xl">
            <header className="flex flex-col md:flex-row justify-between items-end gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Hoàn thành nhiệm vụ để nuôi dưỡng khu vườn tâm hồn của bạn.
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-full border border-blue-100">
                        <span className="font-bold text-blue-700">Level {state.profile?.level || 1}</span>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {/* Sidebar: Quests & Badges */}
                <div className="space-y-6">
                    <QuestList quests={state.quests} />

                    {/* Reuse existing component or build simple badge list if UserEngagement handles basic logic but specific display needed */}
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-sm font-medium text-muted-foreground">Các hoạt động khác</CardTitle>
                        </CardHeader>
