/**
 * GAD-7 Results Page
 * Display anxiety screening results
 */
...
import { GAD7_RECOMMENDATIONS, ANXIETY_HOTLINES } from '@/constants/tests/gad7-questions'
...
export default function GAD7ResultsPage() {
  ...
  useEffect(() => {
    ...
    const storedResult = localStorage.getItem('gad7_result')
    const storedDate = localStorage.getItem('gad7_completed_at')
    ...
    const saveResults = async () => {
      try {
        ...
        await saveMentalHealthRecord({
          ...
          crisisReason: parsedResult.needsCrisisIntervention ? 'Severe anxiety' : undefined,
          completedAt: storedDate ? new Date(storedDate) : new Date(),
        })

        setSaveStatus('saved')
      } catch (error) {
        console.error('Failed to save GAD-7 results:', error)
        setSaveStatus('error')
  ...
  const recommendations = GAD7_RECOMMENDATIONS[
      ...
      <div className="container max-w-5xl mx-auto px-4">
        ...
        <div className="text-center mb-12 relative">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Đánh giá mức độ lo âu trong 2 tuần qua
          </p>
        ...
        {result.needsCrisisIntervention && (
          <Alert variant="destructive" className="mb-8">
            ...
            <AlertTitle className="text-lg font-bold">Cần hỗ trợ chuyên gia</AlertTitle>
        ...
        {/* Score Card */}
        <Card className="glass-card shape-organic-2 mb-8 relative overflow-hidden">
          ...
          <CardContent className="text-center relative z-10">
            <div className="text-6xl font-bold mb-4 bg-gradient-to-br from-foreground to-muted-foreground bg-clip-text text-transparent">{result.totalScore}</div>
            <Badge
              variant="secondary"
              className={`text-2xl px-6 py-3 mb-4 rounded-2xl font-bold ${getSeverityColor(result.severity.color)}`}
            >
            ...
            <div className="relative h-3 bg-muted rounded-full overflow-hidden mt-6 max-w-md mx-auto">
              <div
            ...
            <p className="text-sm font-medium text-muted-foreground mt-3">Thang điểm: 0-21</p>
          </CardContent>
        </Card>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <Button
            onClick={() => router.push('/tests/gad7')}
            variant="outline"
            size="lg"
            className="gap-2"
          >
        ...
        <Card className="glass-card shape-organic-1 mt-8 bg-blue-50 dark:bg-blue-900/20 border-blue-200">
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">
              <strong>Lưu ý:</strong> GAD-7 là công cụ sàng lọc, không phải là công cụ chẩn đoán.
