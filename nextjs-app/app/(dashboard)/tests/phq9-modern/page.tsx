/**
 * PHQ-9 Test Page (Modern UI Version)
 * Patient Health Questionnaire for Depression with Modern Radio Scale
 */
...
import { FREQUENCY_4_SCALE_VI } from '@/components/RadioScale';
import { PHQ9_QUESTIONS, PHQ9_SEVERITY, CRISIS_THRESHOLD } from '@/constants/tests/phq9-questions';

export default function PHQ9ModernTestPage() {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(false);

  // Convert PHQ9_QUESTIONS to ModernQuestion format
  const modernQuestions = PHQ9_QUESTIONS.map(q => ({
    id: q.id,
    question: q.question,
  }));
